...
class Currency(commands.Cog):
    ...
    async def open_account(self, user):
        await self.cursor.execute(f'SELECT user_id FROM main WHERE user_id = {user.id}')
        result1 = await self.cursor.fetchone()
        if result1 is not None:
            await self.cursor.execute(f'CREATE TABLE IF NOT EXISTS u{user.id} (item TEXT, amount INT)')
            return False
        else:
            await self.cursor.execute(f'INSERT INTO main (user_id, wallet, bank, items) VALUES ({user.id}, 0, 0, 0)')
        await self.cursor.execute(f'CREATE TABLE IF NOT EXISTS u{user.id} (item TEXT, amount INT)')
        await self.cursoro.execute(f"CREATE TABLE IF NOT EXISTS u{user.id} (task, status)")
        await self.db.commit()
        await self.cursor.close()
        await self.db.close()
        
        await self.dbo.commit()
        await self.cursoro.close()
        await self.dbo.close()

    async def update_bank(self, user, amt):
        await self.cursor.execute(f'SELECT wallet FROM main WHERE user_id = {user.id}')
        oldamt = await cursor.fetchone()

        await self.cursor.execute(f'UPDATE main SET wallet = {oldamt[0]+amt} WHERE user_id = {user.id}')
        await self.db.commit()
        await self.cursor.close()
        await self.db.close()
    ...
    async def get_amt(self, user):
        await self.cursor.execute(f'SELECT wallet FROM main WHERE user_id = {user.id}')
        walamt = await self.cursor.fetchone()
        
        await self.cursor.execute(f'SELECT bank FROM main WHERE user_id = {user.id}')
        bankamt = await self.cursor.fetchone()
    ...
    async def item_func(self, user, item, amount=None):
        await self.cursor.execute(f"SELECT amount FROM u{user.id} WHERE item = '{item}'")
        oldamount = await self.cursor.fetchone()
        if oldamount is None:
            await self.cursor.execute(f"INSERT INTO u{user.id} (item, amount) VALUES ('{item}', 0)")
            await self.cursor.execute(f"SELECT amount FROM u{user.id} WHERE item = '{item}'")
            oldamount = await self.cursor.fetchone()
        if amount is None:
            return oldamount
        if oldamount is None:
            await self.cursor.execute(f"UPDATE u{user.id} SET amount = {amount} WHERE item = '{item}'")
        else:
            await self.cursor.execute(f"UPDATE u{user.id} SET amount = {amount+oldamount[0]} WHERE item = '{item}'")
        await self.db.commit()
        await self.cursor.close()
        await self.db.close()
    ...
    async def inventory(self, ctx, user:discord.Member=None):
        ...
        await self.cursor.execute(f'SELECT item FROM u{user.id}')
        items = await self.cursor.fetchall()
        for item in items:
            await self.cursor.execute(f"SELECT amount FROM u{user.id} WHERE item = '{item[0]}'")
            amount = await self.cursor.fetchone()
    ...
    async def deposit(self, ctx, amt):
        ...
        await self.open_account(ctx.author)
        if amt == "all":
            await self.cursor.execute(f'UPDATE main SET wallet = 0 WHERE user_id = {ctx.author.id}')
            await self.cursor.execute(f'UPDATE main SET bank = {walamt[0]+bankamt[0]} WHERE user_id = {ctx.author.id}')
            ...
            await self.cursor.execute(f'UPDATE main SET wallet = {walamt[0]-int(amt)} WHERE user_id = {ctx.author.id}')
            await self.cursor.execute(f'UPDATE main SET bank = {int(amt)+bankamt[0]} WHERE user_id = {ctx.author.id}')
    ...
    async def withdraw(self, ctx, amt):
        ...
        if amt == "all":
            await self.cursor.execute(f'UPDATE main SET bank = 0 WHERE user_id = {ctx.author.id}')
            await self.cursor.execute(f'UPDATE main SET wallet = {walamt[0]+bankamt[0]} WHERE user_id = {ctx.author.id}')
            ...
            await self.cursor.execute(f'UPDATE main SET wallet = {walamt[0]+int(amt)} WHERE user_id = {ctx.author.id}')
            await self.cursor.execute(f'UPDATE main SET bank = {bankamt[0]-int(amt)} WHERE user_id = {ctx.author.id}')
    ...
    async def buy(self, ctx, item, quantity=1):
        ...
        if check is not None:
            ...
            await self.cursor.execute(f"SELECT amount FROM u{ctx.author.id} WHERE item = '{item}'")
            check1 = await self.cursor.fetchone()
            walamt, bankamt = await self.get_amt(ctx.author)
            ...
            if walamt[0] + bankamt[0] < price*quantity:
                ...
                if check1 is not None:
                    await self.cursor.execute(f"UPDATE u{ctx.author.id} SET amount = {check1[0] + quantity} WHERE item = '{item}'")
                if check1 is None:
                    await self.cursor.execute(f"INSERT INTO u{ctx.author.id} (item, amount) VALUES ('{item}', {quantity})")
                await self.cursor.execute(f'UPDATE main SET wallet = {walamt[0] - quantity*price} WHERE user_id = {ctx.author.id}')
    ...
    async def sell(self, ctx, item, quantity=1):
        ...
        await self.cursor.execute(f"SELECT item FROM u{ctx.author.id} WHERE item = '{item}'")
        check = await self.cursor.fetchone()
        await self.cursor.execute(f"SELECT amount FROM u{ctx.author.id} WHERE item = '{item}'")
        check1 = await self.cursor.fetchone()
        await self.cursor.execute(f'SELECT wallet FROM main WHERE user_id = {ctx.author.id}')
        ...
        if check == 0:
            ...
            if quantity < 1:
                ...
                await self.cursor.execute(f"UPDATE u{ctx.author.id} SET amount = {check1[0] - quantity} WHERE item = '{item}'")
                await self.cursor.execute(f'UPDATE main SET wallet = {oldamt[0]+sale} WHERE user_id = {ctx.author.id}')
    ...
    async def give(self, ctx, user:discord.Member, mode):
        ...
        if mode == "item":
            ...
            if check is None:
                ...
                await self.cursor.execute(f"SELECT amount FROM u{ctx.author.id} WHERE item = '{msg.content}'")
                check0 = await self.cursor.fetchone()
                if check0 is None:
                    ...
                    await self.cursor.execute(f"SELECT amount FROM u{user.id} WHERE item = '{msg.content}'")
                    useramt = await self.cursor.fetchone()
                    await self.cursor.execute(f"UPDATE u{ctx.author.id} SET amount = {check0[0] - int(msg0.content)} WHERE item = '{msg.content}'")
                    try:
                        await self.cursor.execute(f"UPDATE u{user.id} SET amount = {useramt[0] + int(msg0.content)} WHERE item = '{msg.content}'")
                    except:
                        await self.cursor.execute(f"INSERT INTO u{user.id} (item, amount) VALUES ('{msg.content}', 0)")
                        useramt = 0
                        await self.cursor.execute(f"UPDATE u{user.id} SET amount = {useramt + int(msg0.content)} WHERE item = '{msg.content}'")
    ...
    async def task(self, ctx):
        await self.open_account(ctx.author)
        await self.cursoro.execute(f"SELECT task FROM u{ctx.author.id}")
        result = await self.cursoro.fetchall()
    ...
    async def start(self, ctx, task):
        await self.open_account(ctx.author)
        await self.cursoro.execute(f"CREATE TABLE IF NOT EXISTS u{ctx.author.id} (task, status)")
        await self.cursoro.execute(f"SELECT task FROM u{ctx.author.id} WHERE task = '{task}'")
        result = await self.cursoro.fetchone()
        ...
        if req in msg.content.lower() or msg.content:
            ...
            await self.cursoro.execute(f"INSERT INTO u{ctx.author.id} (task) VALUES ('{task}')")
        await self.db.commit()
        await self.cursor.close()
        await self.db.close()
